...
class PhantomBusterService {
  ...
  getNamedPhantomIds() {
    const connectionsPhantomId = (process.env.CONNECTIONS_EXPORT_PHANTOM_ID || "").trim();
    const searchPhantomId = (
      process.env.SEARCH_EXPORT_PHANTOM_ID ||
      process.env.SEARCH_LEADS_PHANTOM_ID ||
      ""
    ).trim();
    return { connectionsPhantomId, searchPhantomId };
  }

  validateDistinctSearchAndConnectionsPhantoms() {
    const { connectionsPhantomId, searchPhantomId } = this.getNamedPhantomIds();
    if (connectionsPhantomId && searchPhantomId && connectionsPhantomId === searchPhantomId) {
      throw new Error(
        "CONNECTIONS_EXPORT_PHANTOM_ID and SEARCH_EXPORT_PHANTOM_ID are set to the same PhantomBuster agent. Please set different IDs so 'Import My Connections' and 'Lead Search' run their own named phantoms."
      );
    }
  }
  ...
  async exportConnections() {
    try {
      ...
      this.validateDistinctSearchAndConnectionsPhantoms();
      const { connectionsPhantomId } = this.getNamedPhantomIds();
      const phantomId = connectionsPhantomId;
      if (!phantomId) {
  ...
  async searchLeads(searchQuery, numberOfResults) {
    try {
      ...
      const { searchPhantomId } = this.getNamedPhantomIds();
      const phantomId = searchPhantomId;
      if (!phantomId) {
